...
export default class ContestTile extends Component<Props> {
  ...
  renderContestWithMostParticipants(stat: ContestWithMostParticipants) {
    ...
    return (
      <div>
        ...
        The {this.renderContestLink(year)} competition registered the highest number of contestants, {num_participants}!
      </div>
    );
  }
  ...
  renderContestWithMostExAequo(stat: ContestWithMostExAequo) {
    ...
    return (
      <div>
        {this.renderContestImg(year)}
        {num_ex_aequo} is the highest number of ex-aequo, in {this.renderContestLink(year)} all those students ended at
        the first place with the same score!
      </div>
    );
  }
  ...
  renderMostNorthenContest(stat: MostNorthenContest) {
    ...
    return (
      <div>
        ...
        The most northern contest was held in {this.renderContestLink(year)} in {location.location}.
      </div>
    );
  }
  ...
  renderMostSouthernContest(stat: MostSouthernContest) {
    ...
    return (
      <div>
        {this.renderLocation(location)}
        The most southern contest was held in {this.renderContestLink(year)} in {location.location}.
      </div>
    );
  }
  ...
  renderNumBoysGirls(stat: NumBoysGirls) {
    return (
      <AreaChart width={300} height={350} data={stat.num_boys_girls.years} className="card-img-top">
        <CartesianGrid strokeDasharray="3 3" />
        <XAxis dataKey="year" />
        <YAxis width={30} />
        <Tooltip />
        <Area type="monotone" dataKey="num_boys" stackId="1" stroke="#99ebff" fill="#99ebff" name="Boys" />
        <Area type="monotone" dataKey="num_girls" stackId="1" stroke="#ffb3cb" fill="#ffb3cb" name="Girls" />
      </AreaChart>
    );
  }
  ...
  renderNumParticipantsPerYear(stat: NumParticipantsPerYear) {
    return (
      <AreaChart width={300} height={350} data={stat.num_participants_per_year.years} className="card-img-top">
        <CartesianGrid strokeDasharray="3 3" />
        <XAxis dataKey="year" />
        <YAxis width={30} />
        <Tooltip />
  ...
  renderMostUsedLocation(stat: MostUsedLocation) {
    ...
    const years_but_last = years
      .slice(0, -2)
      .map(year => <span key={`most-used-location-${year}`}>{this.renderContestLink(year)}, </span>);
    years_but_last.push(
    ...
    return (
      <div>
        {this.renderLocation(location)}
        {location.location} is the most used location, it has beed used {years.length} times, in {years_but_last} and{" "}
