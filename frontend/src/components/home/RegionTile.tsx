...
export default class RegionTile extends Component<Props> {
  ...
  renderRegionWithMostMedals(region: RegionWithMostMedals) {
    ...
    return (
      <div>
        ...
        {this.renderRegionLink(first)} with {first.num_medals.gold} golds,{" "}
        {first.num_medals.silver} silvers and {first.num_medals.bronze} bronzes.
        At second place there is {this.renderRegionLink(second)} with
        respectivly {second.num_medals.gold}, {second.num_medals.silver} and{" "}
        {second.num_medals.bronze}.
      </div>
    );
  }

  renderRegionWithMostMedalsPerParticipant(
  ...
  renderRegionWithMostFirstPlaces(stat: RegionWithMostFirstPlaces) {
    ...
    return (
      <div>
        ...
        {this.renderRegionLink(region)}, with {region.num_first_places}.
      </div>
    );
  }
  ...
  renderRegionWithMostParticipants(stat: RegionWithMostParticipants) {
    ...
    return (
      <div>
        ...
        {this.renderRegionLink(region)}, with {region.num_participants}.
      </div>
    );
  }
  ...
  render() {
    ...
    let body = null;
    if (kind == "region_with_most_medals")
      body = this.renderRegionWithMostMedals(stat as RegionWithMostMedals);
    else if (kind == "region_with_most_medals_per_participant")
      body = this.renderRegionWithMostMedalsPerParticipant(
        stat as RegionWithMostMedalsPerParticipant
      );
    else if (kind == "region_with_most_first_places")
      body = this.renderRegionWithMostFirstPlaces(
        stat as RegionWithMostFirstPlaces
      );
    else if (kind == "region_with_most_participants")
      body = this.renderRegionWithMostParticipants(
        stat as RegionWithMostParticipants
      );
    else return null;
    return (
